...
local function resolve_my_tag()
    ...
    local myName = gNetworkPlayers[0].name

    if (discordId ~= "0" and discordId ~= "") or (coopnetId ~= "0" and coopnetId ~= "") then
        local assignedTag = 0

        for _, user in ipairs(AUTHORIZED_USERS) do
            if myName == user.name then
                if (user.coopnet ~= "0" and user.coopnet == coopnetId) or 
                   (user.discord ~= "0" and user.discord == discordId) then
                    assignedTag = user.tag
                    break
                end
            end
        end

        gPlayerSyncTable[0].tagId = assignedTag
...
local function get_formatted_tag(playerIndex)
    ...
    if tagId and tagId > 0 and DEF_TAGS[tagId] then
        ...
        local translated_tag = _T and _T(def.name) or def.name
        return def.color .. translated_tag .. " "
    end
    return ""
end
        ...
        if not np.connected then
            ...
            if (s.tagId and s.tagId > 0) or timer <= 0 then
                if network_player_connected_count() > 1 then
                    ...
                    -- Reemplazado texto duro por variable traducible
                    local conn_msg = _T and _T("player_connected") or "¡Se ha conectado!"
                    djui_chat_message_create(displayName .. " \\#ffff50\\" .. conn_msg)
                end
                connectQueue[playerIndex] = nil
            else
...
local function on_player_disconnected(m)
    ...
    if network_player_connected_count() > 0 then
        local displayName = get_player_display_name(m.playerIndex)
        -- Reemplazado texto duro por variable traducible
        local disconn_msg = _T and _T("player_disconnected") or "Se ha desconectado."
        djui_chat_message_create(displayName .. " \\#ffff50\\" .. disconn_msg)
    end
