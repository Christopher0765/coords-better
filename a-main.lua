...
function format_num(val) 
    return tostring(tonumber(string.format("%.5f", tonumber(val) or 0))) 
end
...
local function refresh_chat_command_descriptions()
    ...
    update_chat_command_description("speed", _T("s_cmd_desc"))
    update_chat_command_description("coords", _T("c_cmd_desc"))
    update_chat_command_description("anims", _T("a_cmd_desc"))
    update_chat_command_description("color-change", _T("cc_cmd_desc"))
    update_chat_command_description("lang", _T("lang_cmd_desc"))
    update_chat_command_description("menu", _T("cpmenu_cmd_desc"))
end
...
local function on_hud_render_main()
    ...
    if M.spd or exiting_spd or anim_x_spd > -395 then
        ...
        render_text((_T("spd_x")) .. format_num(m.vel.x), 40 + anim_x_spd, py_base + 20)
        render_text((_T("spd_y")) .. format_num(m.vel.y), 40 + anim_x_spd, py_base + 50)
        render_text((_T("spd_z")) .. format_num(m.vel.z), 40 + anim_x_spd, py_base + 80)
    end
...
local function cmd_toggle_speed(msg)
...
local function cmd_toggle_coords(msg)
    local arg = msg:lower():gsub("%s+", "")
...
local function cmd_toggle_anim(msg)
    local arg = msg:lower():gsub("%s+", "")
...
local function cmd_change_color(msg)
    local col = {}
    for w in string.gmatch(msg, "%S+") do table.insert(col, tonumber(w)) end
    if #col >= 3 and col[1] and col[2] and col[3] then 
        tr = math.max(0, math.min(255, col[1]))
        tg = math.max(0, math.min(255, col[2]))
        tb = math.max(0, math.min(255, col[3]))
        play_select_sound()
        save_config() 
        
        djui_chat_message_create(_T("notify_color") .. " \\#ff0000\\" .. math.floor(tr) .. " \\#00ff00\\" .. math.floor(tg) .. " \\#0000ff\\" .. math.floor(tb))
    else
        play_error_sound()
        djui_chat_message_create(_T("err_rgb"))
    end
    return true
end
...
local function cmd_change_lang(msg)
    ...
    if arg == "es" or arg == "en" or arg == "fr" then
        ...
        if _G.LANG then
            ...
            save_config()
            refresh_chat_command_descriptions()
            djui_chat_message_create(_T("notify_lang"))
        end
        return true
    else
        play_error_sound()
        djui_chat_message_create(_T("err_lang"))
        return true
    end
end

local function cmd_toggle_menu()
...
hook_chat_command("speed", (_T("s_cmd_desc")), cmd_toggle_speed)
hook_chat_command("coords", (_T("c_cmd_desc")), cmd_toggle_coords)
hook_chat_command("anims", (_T("a_cmd_desc")), cmd_toggle_anim)
hook_chat_command("color-change", (_T("cc_cmd_desc")), cmd_change_color)
hook_chat_command("lang", (_T("lang_cmd_desc")), cmd_change_lang)
hook_chat_command("menu", (_T("cpmenu_cmd_desc")), cmd_toggle_menu)

hook_event(HOOK_ON_HUD_RENDER_BEHIND, on_hud_render_main)
